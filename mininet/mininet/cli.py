...
class CLI( Cmd ):
    ...
    def do_iperftest( self, line ):
        """
        iperf test between nodes under demand
        """
        args = line.split()
        if len(args) == 1:
            addr = args[0]
            self.mn.iperfTest(addr)
        else:
            error( 'invalid number of args: iperfTest addr\n '+
                    'addr examples:/home/install/mininet/custom\n' )
